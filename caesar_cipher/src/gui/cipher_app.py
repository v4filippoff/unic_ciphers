...
from src.logic import caesar_cipher
...
def build_window():
    ...
    def showerror_incorrect_language_content():
        messagebox.showerror(title='Ошибка', message='Введите исходный текст в выбранном языке!')

    def showerror_incorrect_key():
        messagebox.showerror(title='Ошибка', message='Ключ может содержать только цифры и знак "-" в начале!')

    def encrypt_action(source_text, key, language_characters):
        encrypted_text = caesar_cipher.encrypt(source_text, key, language_characters)
    ...
    def decrypt_action(source_text, key, language_characters):
        decrypted_text = caesar_cipher.decrypt(source_text, key, language_characters)
    ...
    def cipher_action_button_handler(action_command: Callable):
        def returned_command():
            ...
            if selected_language_value == 'en':
                language_characters = caesar_cipher.LanguageCharacters.EN
            elif selected_language_value == 'ru':
                language_characters = caesar_cipher.LanguageCharacters.RU

            if not caesar_cipher.check_language_identity(source_text, language_characters):
            ...
            return action_command(source_text, int(key), language_characters)
